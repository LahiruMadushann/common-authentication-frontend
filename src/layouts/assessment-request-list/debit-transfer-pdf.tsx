...
export default function DebitTransferPdf(props: { data: InvoiceForPDFType }) {
  ...
  return (
    <div className="notranslate w-full max-h-[80vh] overflow-auto" translate="no">
          ...
          <div className="invoice-container-table flex flex-col md:flex-row gap-4">
            <div className="total-amount-table w-full md:w-1/2">
              <table className="invoice-table w-full mb-4">
                ...
                <tbody> 
                  <tr>
                    <td className="invoice-table-body border p-1">{props.data?.TotalPlusTax >= 0 ? `¥${props.data?.TotalPlusTax.toLocaleString()}` : `-¥${Math.abs(props.data?.TotalPlusTax).toLocaleString()}`}</td>
                  </tr>
                </tbody>
              </table>
            </div>
            <div className="transfer-table w-full md:w-1/2">
              <table className="invoice-table w-full mb-4">
                <thead>
                  <tr>
                    <th className="invoice-table-header border p-1">振替日</th>
          ...
          <div className="responsive-table-container overflow-x-auto">
            <table className="invoice-table w-full mb-4">
              ...
              <tbody>
                <tr>
                  <td className="invoice-table-body border p-1">{props.data?.TotalPlusTax >= 0 ? `¥${props.data?.TotalPlusTax.toLocaleString()}` : `-¥${Math.abs(props.data?.TotalPlusTax).toLocaleString()}`}</td>
                  <td className="invoice-table-body border p-1">{props.data?.netTotal >= 0 ? `¥${props.data?.netTotal.toLocaleString()}` : `-¥${Math.abs(props.data?.netTotal).toLocaleString()}`}</td>
                  <td className="invoice-table-body border p-1">¥{props.data?.tenPercentTax > 0 ? props.data?.tenPercentTax.toLocaleString() : 0}</td>
                  <td className="invoice-table-body border p-1">¥0</td>
                  <td className="invoice-table-body border p-1">¥{props.data?.eightPercentTax > 0 ? props.data?.eightPercentTax.toLocaleString() : 0}</td>
                  <td className="invoice-table-body border p-1">¥{props.data?.taxExempt > 0 ? props.data?.taxExempt.toLocaleString() : 0}</td>
                </tr>
              </tbody>
            </table>
          </div>
            ...
            <table className="invoice-table w-full">
                ...
                {generateBill(
                  ...
                  return (
                    <tr key={index}>
                      ...
                      <td className="border p-1">¥{item.unitPrice.toLocaleString()}</td>
                      <td className="border p-1">{parseFloat(item.total.replace(/[^0-9.-]/g, '')).toLocaleString()}</td>
                    </tr>
                  );
                })}
              </tbody>
              <tfoot>
                <tr>
                  ...
                  <td className="border p-1">{props.data?.netTotal >= 0 ? `¥${props.data?.netTotal.toLocaleString()}` : `-¥${Math.abs(props.data?.netTotal).toLocaleString()}`}</td>
                </tr>
                <tr>
                  <td className="border p-1" colSpan={2}>備考</td>
                  <td className="border p-1" colSpan={1}>消費税（10％）</td>
                  <td className="border p-1">¥{props.data?.tenPercentTax > 0 ? props.data?.tenPercentTax.toLocaleString() : 0}</td>
                </tr>
                <tr>
                  ...
                  <td className="border p-1">¥{props.data?.eightPercentTax > 0 ? props.data?.eightPercentTax.toLocaleString() : 0}</td>
                </tr>
                <tr>
                  <td className="border p-1" colSpan={2}></td>
                  <td className="border p-1" colSpan={1}>非課税</td>
                  <td className="border p-1">¥{props.data?.taxExempt > 0 ? props.data?.taxExempt.toLocaleString() : 0}</td>
                </tr>
              </tfoot>
            </table>
          </div>
      ...
      <DialogFooter className="mt-4">
        <Button onClick={generateInvoicePDF}>PDFの生成</Button>
      </DialogFooter>
    </div>
  );
}
